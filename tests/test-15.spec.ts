...
test("Test-12: Search and Navigate to J&R Custom Woodwork (Cross-Device) with Random Context", async ({
  browser,
}) => {
  // Log the test number at the start
  console.log("Running Test-12: Search and Navigate to J&R Custom Woodwork");
  ...
  if (found) {
    ...
    if (shouldOpenFacebook()) {
      ...
      await page.getByLabel("Instagram page").click();
      const facebookPopup = await facebookPopupPromise;

      await page.waitForTimeout(timeGenerator.waitOneToThreeTime());
      await facebookPopup.close();
    } else {
      console.log("Skipping Instagram interaction");
    }
